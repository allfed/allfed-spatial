...
# Modify these
GHBU_UNAME=${GHBU_UNAME-"<add github username>"}                             
GHBU_PASSWD=${GHBU_PASSWD-"<add personal access token>"}   # https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token

# Leave everything else as-is, unless you'd like to change default parameters
GHBU_BACKUP_DIR=${GHBU_BACKUP_DIR-"allfed-github-backups"}              
GHBU_ORG=${GHBU_ORG-"allfed"}                               
...
REPOLIST=`check curl --silent -u $GHBU_UNAME:$GHBU_PASSWD ${GHBU_API}/orgs/${GHBU_ORG}/repos\?per_page=100 -q | check grep "\"full_name\"" | check awk -F': "allfed/' '{print $2}' | check sed -e 's/",//g'`
